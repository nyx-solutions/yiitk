    ...
    class InflectorHelper extends Inflector
    {
        ...
        public static function timeElapsed($fromTime = null, $human = true, $toTime = null, $append = null)
        {
            ...
            if ($fromTime != null) {
                ...
                if (empty($append)) {
                    $append = ' '.Yii::t('sbinflector', 'ago');
                }
                if ($human) {
                    // now we just find the difference
                    if ($diff <= 0) {
                        $elapsed = Yii::t('sbinflector', 'a moment ago');
                    } elseif ($diff < 60) {
                        $elapsed = Yii::t('sbinflector', '{n, plural, one{one second} other{# seconds}}',
                                ['n' => $diff]).$append;
                    } elseif ($diff >= 60 && $diff < $intervals['hour']) {
                        $diff = floor($diff / $intervals['minute']);
                        $elapsed = Yii::t('sbinflector', '{n, plural, one{one minute} other{# minutes}}',
                                ['n' => $diff]).$append;
                    } elseif ($diff >= $intervals['hour'] && $diff < $intervals['day']) {
                        $diff = floor($diff / $intervals['hour']);
                        $elapsed = Yii::t('sbinflector', '{n, plural, one{one hour} other{# hours}}',
                                ['n' => $diff]).$append;
                    } elseif ($diff >= $intervals['day'] && $diff < $intervals['week']) {
                        $diff = floor($diff / $intervals['day']);
                        $elapsed = Yii::t('sbinflector', '{n, plural, one{one day} other{# days}}',
                                ['n' => $diff]).$append;
                    } elseif ($diff >= $intervals['week'] && $diff < $intervals['month']) {
                        $diff = floor($diff / $intervals['week']);
                        $elapsed = Yii::t('sbinflector', '{n, plural, one{one week} other{# weeks}}',
                                ['n' => $diff]).$append;
                    } elseif ($diff >= $intervals['month'] && $diff < $intervals['year']) {
                        $diff = floor($diff / $intervals['month']);
                        $elapsed = Yii::t('sbinflector', '{n, plural, one{one month} other{# months}}',
                                ['n' => $diff]).$append;
                    } elseif ($diff >= $intervals['year']) {
                        $diff = floor($diff / $intervals['year']);
                        $elapsed = Yii::t('sbinflector', '{n, plural, one{one year} other{# years}}',
                                ['n' => $diff]).$append;
                    }
                } else {
        ...
        {
            if (!empty(Yii::$app->i18n->translations['sbinflector'])) {
                return;
            }

            Yii::setAlias('@sbinflector', __DIR__);

            Yii::$app->i18n->translations['sbinflector'] = [
                'class'            => 'yii\i18n\PhpMessageSource',
                'basePath'         => '@sbinflector/messages',
        ...
        public static function numToWords($num)
        {
            ...
            if ($num < 0) {
                return Yii::t('sbinflector', 'minus').' '.static::convertTri(-$num, 0);
            }

            if ($num == 0) {
                return Yii::t('sbinflector', 'zero');
            }
        ...
        protected static function convertTri($num, $tri)
        {
            ...
            // do hundreds
            if ($x > 0) {
                $str = $ones[$x].' '.Yii::t('sbinflector', 'hundred');
            }
        ...
        public static function ones()
        {
            ...
            return [
                '',
                ' '.Yii::t('sbinflector', 'one'),
                ' '.Yii::t('sbinflector', 'two'),
                ' '.Yii::t('sbinflector', 'three'),
                ' '.Yii::t('sbinflector', 'four'),
                ' '.Yii::t('sbinflector', 'five'),
                ' '.Yii::t('sbinflector', 'six'),
                ' '.Yii::t('sbinflector', 'seven'),
                ' '.Yii::t('sbinflector', 'eight'),
                ' '.Yii::t('sbinflector', 'nine'),
                ' '.Yii::t('sbinflector', 'ten'),
                ' '.Yii::t('sbinflector', 'eleven'),
                ' '.Yii::t('sbinflector', 'twelve'),
                ' '.Yii::t('sbinflector', 'thirteen'),
                ' '.Yii::t('sbinflector', 'fourteen'),
                ' '.Yii::t('sbinflector', 'fifteen'),
                ' '.Yii::t('sbinflector', 'sixteen'),
                ' '.Yii::t('sbinflector', 'seventeen'),
                ' '.Yii::t('sbinflector', 'eighteen'),
                ' '.Yii::t('sbinflector', 'nineteen')
            ];
        }
        ...
        public static function tens()
        {
            ...
            return [
                '',
                '',
                ' '.Yii::t('sbinflector', 'twenty'),
                ' '.Yii::t('sbinflector', 'thirty'),
                ' '.Yii::t('sbinflector', 'forty'),
                ' '.Yii::t('sbinflector', 'fifty'),
                ' '.Yii::t('sbinflector', 'sixty'),
                ' '.Yii::t('sbinflector', 'seventy'),
                ' '.Yii::t('sbinflector', 'eighty'),
                ' '.Yii::t('sbinflector', 'ninety')
            ];
        }
        ...
        public static function months()
        {
            ...
            return [
                1 => Yii::t('sbinflector', 'January'),
                Yii::t('sbinflector', 'February'),
                Yii::t('sbinflector', 'March'),
                Yii::t('sbinflector', 'April'),
                Yii::t('sbinflector', 'May'),
                Yii::t('sbinflector', 'June'),
                Yii::t('sbinflector', 'July'),
                Yii::t('sbinflector', 'August'),
                Yii::t('sbinflector', 'September'),
                Yii::t('sbinflector', 'October'),
                Yii::t('sbinflector', 'November'),
                Yii::t('sbinflector', 'December'),
            ];
        }
        ...
        public static function days()
        {
            ...
            return [
                1 => Yii::t('sbinflector', 'Sunday'),
                Yii::t('sbinflector', 'Monday'),
                Yii::t('sbinflector', 'Tuesday'),
                Yii::t('sbinflector', 'Wednesday'),
                Yii::t('sbinflector', 'Thursday'),
                Yii::t('sbinflector', 'Friday'),
                Yii::t('sbinflector', 'Saturday')
            ];
        }
        ...
        public static function triplets()
        {
            ...
            return [
                '',
                ' '.Yii::t('sbinflector', 'thousand'),
                ' '.Yii::t('sbinflector', 'million'),
                ' '.Yii::t('sbinflector', 'billion'),
                ' '.Yii::t('sbinflector', 'trillion'),
                ' '.Yii::t('sbinflector', 'quadrillion'),
                ' '.Yii::t('sbinflector', 'quintillion'),
                ' '.Yii::t('sbinflector', 'sextillion'),
                ' '.Yii::t('sbinflector', 'septillion'),
                ' '.Yii::t('sbinflector', 'octillion'),
                ' '.Yii::t('sbinflector', 'nonillion'),
            ];
        }
         ...
         * @param boolean|int $start the first day or month to set. Defaults to `1`.
         ...
         * @param boolean $showLast         whether to show the last date (set in $to) even if it does not match
         *                                  interval.
         *
         * @return array
         * @throws InvalidConfigException
        ...
        public static function boolList($false = null, $true = null)
        {
            ...
            return [
                false => empty($false) ? Yii::t('sbinflector', 'No') : $false,
                // == 0
                true  => empty($true) ? Yii::t('sbinflector', 'Yes') : $true,
                // == 1
            ];
        }
        ...
        public static function getBrowser($common = false, $browsers = [], $agent = null)
        {
            ...
            if ($common) {
                $browsers = [
                    "opera"         => Yii::t('sbinflector', "Opera"),
                    "chrome"        => Yii::t('sbinflector', "Google Chrome"),
                    "safari"        => Yii::t('sbinflector', "Safari"),
                    "firefox"       => Yii::t('sbinflector', "Mozilla Firefox"),
                    "msie"          => Yii::t('sbinflector', "Microsoft Internet Explorer"),
                    "mobile safari" => Yii::t('sbinflector', "Mobile Safari"),
                ];
            } elseif (empty($browsers)) {
                $browsers = [
                    "opera"         => Yii::t('sbinflector', "Opera"),
                    "maxthon"       => Yii::t('sbinflector', "Maxthon"),
                    "seamonkey"     => Yii::t('sbinflector', "Mozilla Sea Monkey"),
                    "arora"         => Yii::t('sbinflector', "Arora"),
                    "avant"         => Yii::t('sbinflector', "Avant"),
                    "omniweb"       => Yii::t('sbinflector', "Omniweb"),
                    "epiphany"      => Yii::t('sbinflector', "Epiphany"),
                    "chromium"      => Yii::t('sbinflector', "Chromium"),
                    "galeon"        => Yii::t('sbinflector', "Galeon"),
                    "puffin"        => Yii::t('sbinflector', "Puffin"),
                    "fennec"        => Yii::t('sbinflector', "Mozilla Firefox Fennec"),
                    "chrome"        => Yii::t('sbinflector', "Google Chrome"),
                    "mobile safari" => Yii::t('sbinflector', "Mobile Safari"),
                    "safari"        => Yii::t('sbinflector', "Apple Safari"),
                    "firefox"       => Yii::t('sbinflector', "Mozilla Firefox"),
                    "iemobile"      => Yii::t('sbinflector', "Microsoft Internet Explorer Mobile"),
                    "msie"          => Yii::t('sbinflector', "Microsoft Internet Explorer"),
                    "konqueror"     => Yii::t('sbinflector', "Konqueror"),
                    "amaya"         => Yii::t('sbinflector', "Amaya"),
                    "netscape"      => Yii::t('sbinflector', "Netscape"),
                    "mosaic"        => Yii::t('sbinflector', "Mosaic"),
                    "netsurf"       => Yii::t('sbinflector', "NetSurf"),
                    "netfront"      => Yii::t('sbinflector', "NetFront"),
                    "minimo"        => Yii::t('sbinflector', "Minimo"),
                    "blackberry"    => Yii::t('sbinflector', "Blackberry"),
                ];
            }
