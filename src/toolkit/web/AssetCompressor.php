<?php
    ...
    class AssetCompressor extends Component implements BootstrapInterface
    {
            ...
            if ($app instanceof Application) {
                ...
                $app->response->on(
                    ...
                    function (Event $event) use ($app) {
                        ...
                        if ($this->enabled && ($this->htmlFormatter instanceof IAssetHtmlFormatter) && $response->format === Response::FORMAT_HTML && !$app->request->isAjax && !$app->request->isPjax && !empty($response->data)) {
         ...
         * @throws YiiException
         */
        protected function process(View $view): void
        {
         ...
         * @throws YiiException
         */
        protected function processJsFiles(array $files = []): array
        {
            ...
            try {
                ...
                foreach ($files as $fileCode => $fileTag) {
                    if (Url::isRelative($fileCode)) {
                        ...
                        $jsFileRemouteCompile = $this->jsFileRemouteCompile;

                        if ($jsFileRemouteCompile) {
            ...
            if ($resultContent) {
                ...
                if (!is_dir($rootDir) && !FileHelper::createDirectory($rootDir, 0777)) {
                    return $files;
                }

                if ($this->jsFileCompress) {
                    $content = Minifier::minify($content, ['flaggedComments' => $this->jsFileCompressFlaggedComments]);
                }
         ...
         * @throws Exception
         */
        protected function processJs(?array $parts): array
        {
            $result = [];

            if ($parts) {
                foreach ($parts as $key => $value) {
                    $result[$key] = Minifier::minify($value, ['flaggedComments' => $this->jsCompressFlaggedComments]);
                }
            }
         ...
         * @throws YiiException
         */
        protected function processCssFiles(array $files = []): array
        {
            ...
            try {
                ...
                foreach ($files as $fileCode => $fileTag) {
                    if (Url::isRelative($fileCode)) {
                        ...
                        $cssFileRemouteCompile = $this->cssFileRemouteCompile;

                        if ($cssFileRemouteCompile) {
                            //Try to download the deleted file
         ...
         * @return array
         */
        protected function processCss(array $css = []): array
        {
            $newCss = [];
            ...
            $resultCss = implode("\n", $newCss);
            $resultCss = CssMin::minify($resultCss);

            return [md5($resultCss) => "<style>".$resultCss."</style>"];
        }
         ...
         * @throws YiiException
         */
        protected function readLocalFile($filePath): string
        {
            if (YII_ENV === 'dev') {
            ...
            if (!is_file($filePath)) {
                throw new YiiException("Read file error '{$filePath}'");
            }

            $file = fopen($filePath, 'rb');

            if (!$file) {
                throw new YiiException("Unable to open file: '{$filePath}'");
            }
         ...
         */
        protected function fileGetContents($file): string
        {
            ...
            throw new YiiException("File get contents '{$file}' error: ".$response->content);
        }
        //endregion
    }
